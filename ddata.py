...
ddata.py module used with earlier implementations but a number
of methods still used in CCA3

contains class MapData -- instantiation of this class
occurs each mission, ie, simulation
...
many of the methods related to navigation processing in ddata.py have now
been moved to module hdata.py
...
try:
    ...
    print('\nprogram will end -- ddata module of causal cog arch unable to import standard lib module')
...
try:
    ...
    print('\nprogram will end -- ddata.py module of the causal cog arch unable to import a PyPI module')
...
try:
    ...
    print('program will end -- ddata.py module of the causal cog arch unable to import a third-party module')
    print('please check requirements.txt and install all required dependencies')
    sys.exit()
#
...
try:
    ...
    print('program will end -- hdata.py module unable to import a causal cognitive architecture module')
    print('please check requirements.txt and install all required dependencies')
    sys.exit()
#
#
...
class MapData:
    '''many of the data variables  and associated methods are held here in
    Mapdata class
    '''
    def __init__(self):
        '''these data variables are initiated at start of each mission (ie,
           ...
           be aware the following are initiated at program start via instant-
                iation of ddata.MultipleSessionsData -- 'g' instance:
                    raw_future_expected_values_from_multiple_missions: list = []
                    conscious_memory = [['start of conscious_memory']]
                    sensory_buffer: list = []
        '''
        #print('debug: MapData is being instantiated -- if to d make sure appropriate -- data of previous mission will be erased')
        self.debug = DEBUG
        ...
        self.visual_dict = {'11100011':'lake', '01010000':'lost hiker visual', '11111100':'obstruction', '00010001':'shallow river', '00011001':'shallow river + spraying water',
                            '11000110':'forest'}
        self.vis_features = self.visual_dict = {'11100011':'xxxxxxxxxlake', '01010000':'lost hiker visual', '11111100':'obstruction', '00010001':'shallow river', '00011001':'shallow river + spraying water',
                            '11000110':'forest'}
        ...
        self.aud_features =  {'00000000':'xxxxxxxstrange silence', '11000000':'forest_noise', '11110000':'human cry help', '00010001':'smooth water sound', '11100000':'bird mating call', '01010101':'water spray noise'}
    ...
    def load_first_scene_into_current_navmap(self, h, first_scene = 'NOT_SPECIFIED'):
        '''ensures h.first_scene exists in the Causal Memory Module
        near future work will allow any first scene to be chosen, including
        those that do not exists
        near future work will also allow rapid switching between scenes, but
        at time of writing after a sensory scene/mission processing is complete
        return is transferred back to the cca3.py module

        input parameters:
            d, first_scene (which is usually h.first_scene)
        returns:
            h since modified by this method
        '''
        if self.debug == 'avoid pylint no-self-use':
            print()
        if first_scene not in ('PATIENT', 'FOREST', 'SUDOKO'):
            print('\nat present processing completely new sensory scenes is not implemented....')
            print('this is planned for the near future....')
            print('first sensory scene thus defaulting to patient in hospital room\n')
            first_scene = 'PATIENT'
        if first_scene not in 'PATIENT':
            print('\nfor developmental reasons at present')
            print('first sensory scene defaults to patient in hospital room\n')
            first_scene = 'PATIENT'
        h.current_map = 8
        return h
